  ...
  <tr className="border-b">
    <td className="w-48 hover:bg-gray-200 p-2 flex flex-col space-y-1">
      {order.allProduct.map((product, i) => {
        // Ensure product.id and product.id.pImages exist before trying to access them
        return (
          <span className="block flex items-center space-x-2" key={i}>
            {product.id && product.id.pImages && product.id.pImages.length > 0 ? (
              <img
              ...
              <span>No Image Available</span> // Fallback if no image is available
            )}
            <span>{product.id ? product.id.pName : "No Name Available"}</span>
